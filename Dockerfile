...
RUN groupadd -r appuser && useradd -r -g appuser -d /app -s /sbin/nologin appuser \
    && mkdir -p /app/logs /app/skills /app/knowledge /app/user_profiles /app/memory \
    && chown -R appuser:appuser /app

# 复制应用代码
COPY --chown=appuser:appuser app.py config.py database.py agents.py tools.py models.py mcp_client.py skill_manager.py knowledge_base.py alert_monitor.py user_profile.py memory_store.py ./
